...
#  +++++ (c) cra
#  +++++ Leider 2 Hardcoded Sachen drin
#  +++++
#  +++++	1. env:Path muss c:\support\enable-thumbs sein (deswegen muss der Ordner da auch hinkopiert werden)
#  +++++	2. lgpo.exe /t braucht den Pfad für die Regsitry .txt auch als vollen Pfad 
#  +++++
#  +++++	Trotzdem spassig :-)
...
function Write-ProgressHelper { # https://adamtheautomator.com/write-progress/
	...
	Write-Progress -Activity "Enable Thumbs Preview" -Status $Message -PercentComplete (($StepNumber / $steps) * 100)
}
...
Write-Host ""
Write-Host ":-)"
Write-ProgressHelper -Message 'Erstelle PATH Variable' -StepNumber ($stepCounter++) 
$env:Path=$env:Path + ';C:\support\enable-thumbs'
Write-ProgressHelper -Message 'Erstelle HKEY_USERS Hive' -StepNumber ($stepCounter++) 
#  +++++ Erstelle PSDrives
New-PSDrive HKU Registry HKEY_USERS | Out-Null
Write-ProgressHelper -Message 'Erstelle HKEY_CLASSES_ROOT Hive' -StepNumber ($stepCounter++)
...
New-ItemProperty -Path ".\DWM" -Name "AlwaysHibernateThumbnails" -Value "0"  -PropertyType "DWORD" | Out-Null
...
New-ItemProperty -Path ".\Explorer" -Name "NoThumbnailCache" -Value "0"  -PropertyType "DWORD" | Out-Null
...
New-ItemProperty -Path ".\Explorer" -Name "DisableThumbsDBOnNetworkFolders" -Value "0"  -PropertyType "DWORD" | Out-Null
...
New-ItemProperty -Path ".\Explorer" -Name "DisableThumbnails" -Value "0"  -PropertyType "DWORD" | Out-Null
...
Write-Host "Hooray! Half way through!!!"
Write-ProgressHelper -Message 'Schreibe Keys' -StepNumber ($stepCounter++)
...
#  +++++ LGPO.exe call
& lgpo.exe /t C:\support\enable-thumbs/mlgpo.txt
& lgpo.exe /t C:\support\enable-thumbs/ulgpo.txt
Write-Host
Write-ProgressHelper -Message 'Beende explorer.exe' -StepNumber ($stepCounter++)
