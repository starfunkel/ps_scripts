...
Function reg2ps1 {
    ...
    Process {
        ...
        foreach($File in $Files) {
                    ...
                    switch ($key.StartsWith("-HKEY"))
                    {
                        ...
                        $false {
                            ...
                            if ($addedpath -notcontains $key) {
                                $Commands += 'New-Item -Path {0} -Force '-f $key        #### added -force, deleted -ErrorAction SilentlyContinue | Out-Null
                                $addedpath+=$key
                            }
                        }
                    }
            ...
            $Commands | out-file "${parent}\${filename}_reg.ps1" -encoding $encoding
        }
        if ($isfolder -eq $true) {
            $allcommands=(get-childitem $path -recurse -force -file -filter "*_reg.ps1").fullname | where-object {$_ -notmatch "allcommands_reg"} | foreach-object {get-content $_}
            $allcommands | out-file "${path}\allcommands_reg.ps1" -encoding $encoding
        }
    }
}
